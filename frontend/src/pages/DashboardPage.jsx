  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        // All numbers are already rounded to 2 decimals from backend
        setStats({
      ...
      <div className="p-4 md:p-6 space-y-6">
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            ...
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400"><AnimatedCounter value={stats.cashBalance} decimals={2} suffix=" TND" /></p>
          </motion.div>
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Bank Balance</h3>
            <p className="text-2xl font-bold text-purple-600 dark:text-purple-400"><AnimatedCounter value={stats.bankBalance} decimals={2} suffix=" TND" /></p>
          </motion.div>
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Monthly Income</h3>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400"><AnimatedCounter value={stats.monthlyIncome} decimals={2} suffix=" TND" /></p>
          ...
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            ...
            <p className="text-2xl font-bold text-red-600 dark:text-red-400"><AnimatedCounter value={stats.monthlyExpenses} decimals={2} suffix=" TND" /></p>
          ...
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            ...
            <p className={`text-2xl font-bold ${stats.monthlySavings >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}><AnimatedCounter value={stats.monthlySavings} decimals={2} suffix=" TND" /></p>
          </motion.div>
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg Daily Spending</h3>
            <p className="text-2xl font-bold text-gray-900 dark:text-white"><AnimatedCounter value={stats.avgDailySpending} decimals={2} suffix=" TND" /></p>
          </motion.div>
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Largest Expense</h3>
            <p className="text-2xl font-bold text-gray-900 dark:text-white"><AnimatedCounter value={stats.largestExpense} decimals={2} suffix=" TND" /></p>
          </motion.div>
          <motion.div variants={item} className="card p-4 hover:shadow-lg transition">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Net Cash Flow</h3>
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400"><AnimatedCounter value={stats.netCashflow} decimals={2} suffix=" TND" /></p>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card p-4">
            ...
            {stats.topCategories.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No spending data yet. Add some expenses!</p>
            ) : (
          ...
          <div className="card p-4">
            ...
            {stats.monthlyTrend.length === 0 ? (
              <p className="text-gray-500 text-center py-8">Not enough historical data.</p>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card p-4">
            ...
            {stats.recentTransactions.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No recent transactions. Add one!</p>
            ) : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {stats.recentTransactions.map(tx => (
                  <div key={tx.id} className="flex justify-between border-b border-gray-200 dark:border-gray-700 py-2">
                    <span className="text-sm text-gray-600 dark:text-gray-400">{tx.description || tx.category || 'Transaction'}</span>
